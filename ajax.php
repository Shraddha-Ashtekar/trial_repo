...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($mysqli, $sql)) >= 1) {
            $response['done'] = false;
...
if (isset($_POST['room_type'])) {
    ...
    $get_room_sql = "SELECT * FROM room WHERE room_type_id = '$room_type_id' AND status IS NULL AND deleteStatus = '0'";
    $get_room_result = mysqli_query($mysqli,$get_room_sql);
    if ($get_room_result) {
        echo "<option selected disabled>Select Room Type</option>";
        while ($room = mysqli_fetch_assoc($get_room_result)) {
...
if (isset($_POST['room_price'])) {
    ...
    $get_room_sql = "SELECT T.price FROM room R INNER JOIN room_type T ON R.room_type_id = T.room_type_id WHERE R.room_id = '+$room_id+'";
    $get_room_result = mysqli_query($mysqli,$get_room_sql);
    if ($get_room_result) {
        $room = mysqli_fetch_assoc($get_room_result);
        echo $room['price'];
    } else {
        echo "0";
    }
